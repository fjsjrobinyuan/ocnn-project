...
/**
 * CORRECTED testbench for 14-layer MinkowskiNet using persistent BRAM + pruned DRAM architecture
 */
int main() {
    std::cout << "=== 14-LAYER MINKOWSKINET TESTBENCH (CORRECTED) ===" << std::endl;
    ...
    // Initialize 14-layer weights and biases
    std::cout << "Initializing 14 layers of weights and biases..." << std::endl;
    
    static float layer_weights[14][KERNEL_VOLUME][MAX_FEATURE_CHANNELS][MAX_FEATURE_CHANNELS];
    static float layer_biases[14][MAX_FEATURE_CHANNELS];
    ...
    for (int layer = 0; layer < 14; layer++) {
        ...
        std::cout << "  Layer " << (layer + 1) << ": " << config.input_channels 
                  ...
                  << " (" << (config.is_transpose ? "ConvTranspose" : "Conv") << ")" << std::endl;
    ...
    const ap_uint<32> MAX_PRUNED_SIZE = occupied_voxel_count * MAX_FEATURE_CHANNELS;
    ...
    // Full cubic output (64³ × 20 channels)
    const ap_uint<32> FULL_OUTPUT_SIZE = 64 * 64 * 64 * 20;
    ...
    std::cout << "\n=== RUNNING 14-LAYER MINKOWSKINET WITH PERSISTENT ACCELERATOR ===" << std::endl;
    
    ap_uint<32> total_processed_voxels = 0;
    
    // Call the corrected 14-layer MinkowskiNet pipeline
    ...
    // Analyze final full cubic output (64³ × 20 classes)
    std::cout << "\nAnalyzing final full cubic output (64³ × 20 classes)..." << std::endl;
    ...
    std::cout << "  Total output size: 64³ × 20 = " << (64*64*64*20) << " values" << std::endl;
    ...
    std::cout << "  Sparsity preserved: " << (non_zero_outputs < 64*64*64*20/4 ? "YES" : "NO") << std::endl;
    ...
    // Save results
    std::cout << "\nSaving results to minkowski_net_results_corrected.txt..." << std::endl;
    std::ofstream results("minkowski_net_results_corrected.txt");
    results << "14-Layer MinkowskiNet Results (CORRECTED ARCHITECTURE)\n";
    results << "=====================================================\n";
    results << "Input: 64³ voxels, 3 channels (" << occupied_voxel_count << " occupied, " 
            << (100.0f * occupied_voxel_count / (64*64*64)) << "% sparse)\n";
    results << "Output: 64³ voxels, 20 classes (with zeros restored)\n";
    ...
    results << "- Persistent BRAM octree across all 14 layers\n"; 
    ...
    std::cout << "Your key innovations are now properly utilized!" << std::endl;
