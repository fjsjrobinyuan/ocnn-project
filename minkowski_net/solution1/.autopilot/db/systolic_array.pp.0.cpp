...
STORE_WEIGHTS:
    ...
    {
        VITIS_LOOP_81_1: for (int i = 0; i < 32; i++)
        {
#pragma HLS PIPELINE II = 1
 VITIS_LOOP_84_2: for (int j = 0; j < 32; j++)
            {
...
#pragma HLS PIPELINE II = 1
 PE_COMPUTE:
        ...
        {
#pragma HLS UNROLL
 VITIS_LOOP_373_1: for (int j = 0; j < 32; j++)
            {
#pragma HLS UNROLL
 if (k >= j && k - j < 32 && (k - j) == i)
                {
...
#pragma HLS UNROLL
 float sum = 0;
        VITIS_LOOP_389_2: for (int i = 0; i < 32; i++)
        {
#pragma HLS UNROLL
...
RESET_PE:
    ...
    {
#pragma HLS UNROLL
 VITIS_LOOP_405_1: for (int j = 0; j < 32; j++)
        {
#pragma HLS UNROLL
 pe_array[i][j].accumulator = 0;
        }
...
STORE_WEIGHTS:
    for (int n = 0; n < (3 * 3 * 3); n++)
    {
        VITIS_LOOP_419_1: for (int i = 0; i < 32; i++)
        {
#pragma HLS PIPELINE II = 1
 VITIS_LOOP_422_2: for (int j = 0; j < 32; j += 8)
            {
#pragma HLS UNROLL factor = 8
 VITIS_LOOP_425_3: for (int k = 0; k < 8 && (j + k) < 32; k++)
                {
#pragma HLS UNROLL
...
#pragma HLS ARRAY_PARTITION variable = partial_sums complete
 VITIS_LOOP_491_1: for (int f = 0; f < 32; f++)
    {
#pragma HLS UNROLL
 partial_sums[f] = stored_bias[f];
    }
