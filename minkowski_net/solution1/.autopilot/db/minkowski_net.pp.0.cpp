...
# 2 "<built-in>" 2
# 1 "minkowski_net.cpp" 2
# 1 "./minkowski_net_14_layer.h" 1
...
# 47 "./minkowski_net_14_layer.h"
__attribute__((sdx_kernel("minkowski_net_14_layer_pipeline", 0))) void minkowski_net_14_layer_pipeline(
    hls::stream<VoxelData> &input_voxel_stream,
    ap_uint<32> *final_output_full_cubic,


    float layer_weights[14][(3 * 3 * 3)][1024][1024],
    float layer_biases[14][1024],
...
# 2 "minkowski_net.cpp" 2
# 17 "minkowski_net.cpp"
__attribute__((sdx_kernel("minkowski_net_14_layer_pipeline", 0))) void minkowski_net_14_layer_pipeline(
    hls::stream<VoxelData> &input_voxel_stream,
    ap_uint<32> *final_output_full_cubic,
...
#pragma HLS STREAM variable=early_trigger_stream depth=64
 ...
 ap_uint<32> initial_processed_voxels = 0;
    ...
    VITIS_LOOP_96_1: while (!feature_data_stream.empty() && !write_addr_stream.empty()) {
#pragma HLS PIPELINE II=1
 VoxelData voxel = feature_data_stream.read();
        ap_uint<60> morton_addr = write_addr_stream.read();


        VITIS_LOOP_102_2: for (int c = 0; c < 3; c++) {
...
#pragma HLS INLINE off
 ...
 static float z_buffer[3][(8 * 8 * 8)][1024];
...
#pragma HLS UNROLL factor=4
 output_features[oc] = bias[oc];
        ...
        KERNEL_CONV: for (int k = 0; k < (3 * 3 * 3); k++) {
#pragma HLS PIPELINE II=1
 ...
 ap_int<8> dx = NEIGHBOR_OFFSETS[k][0];
            ...
            ap_uint<32> neighbor_x = 0, neighbor_y = 0, neighbor_z = 0;
...
#pragma HLS UNROLL
 neighbor_features[ic] = 0.0f;
            ...
            CONV_ACCUMULATE: for (int oc = 0; oc < config.output_channels; oc++) {
                VITIS_LOOP_288_1: for (int ic = 0; ic < config.input_channels; ic++) {
#pragma HLS PIPELINE II=1
...
#pragma HLS PIPELINE II=1
 ap_uint<32> sorted_dram_addr = v * config.output_channels + c;
...
#pragma HLS PIPELINE II=1
 full_cubic_output[i] = 0;
    ...
    RECONSTRUCT_FULL_CUBIC: for (int z = 0; z < 64; z++) {
        VITIS_LOOP_417_1: for (int y = 0; y < 64; y++) {
            VITIS_LOOP_418_2: for (int x = 0; x < 64; x++) {
#pragma HLS PIPELINE II=1
 ...
 bool voxel_exists = false;
                ...
                if (voxel_exists && pruned_idx < num_pruned_voxels) {
                    ...
                    VITIS_LOOP_429_3: for (int c = 0; c < output_channels; c++) {
#pragma HLS UNROLL factor=4
