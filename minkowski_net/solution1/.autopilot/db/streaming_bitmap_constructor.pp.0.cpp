...
#pragma HLS INTERFACE s_axilite port = return
 static ap_uint<32> voxel_count = 0;
    ...
    static ap_uint<1> L1_temp[(8 / 2) * (8 / 2) * (8 / 2)];
    static ap_uint<1> L2_temp[((8 / 2) / 2) * ((8 / 2) / 2) * ((8 / 2) / 2)];
...
#pragma HLS BIND_STORAGE variable = L3_temp type = ram_1p impl = bram
 static ap_uint<1> initialized = 0;
    if (!initialized)
    {
        ...
        l1_write_pos = 0;
        l2_write_pos = 0;
        VITIS_LOOP_105_1: for (int i = 0; i < (8 / 2) * (8 / 2) * (8 / 2); i++)
        {
            L1_temp[i] = 0;
        }
        VITIS_LOOP_109_2: for (int i = 0; i < ((8 / 2) / 2) * ((8 / 2) / 2) * ((8 / 2) / 2); i++)
        {
            L2_temp[i] = 0;
        }
        L3_temp[0] = 0;
...
L2_CONSTRUCTION:
    for (int l2_z = 0; l2_z < ((8 / 2) / 2); l2_z++)
    {
        VITIS_LOOP_134_3: for (int l2_y = 0; l2_y < ((8 / 2) / 2); l2_y++)
        {
#pragma HLS PIPELINE II = 1
 VITIS_LOOP_137_4: for (int l2_x = 0; l2_x < ((8 / 2) / 2); l2_x++)
            {
#pragma HLS UNROLL
 bool l2_has_data = false;
            ...
            L1_BLOCK_PROCESSING:
                ...
                {
                    VITIS_LOOP_145_5: for (int dy = 0; dy < 2; dy++)
                    {
                        VITIS_LOOP_147_6: for (int dx = 0; dx < 2; dx++)
                        {
#pragma HLS UNROLL
 int l1_x = l2_x * 2 + dx;
                            int l1_y = l2_y * 2 + dy;
                            int l1_z = l2_z * 2 + dz;
                            int l1_idx = l1_z * (8 / 2) * (8 / 2) + l1_y * (8 / 2) + l1_x;
                ...
                int l2_idx = l2_z * ((8 / 2) / 2) * ((8 / 2) / 2) + l2_y * ((8 / 2) / 2) + l2_x;
                L2_temp[l2_idx] = l2_has_data ? 1 : 0;
                if (l2_has_data)
                {
                    VITIS_LOOP_165_7: for (int i = 0; i < 8; i++)
                    {
    ...
    VITIS_LOOP_176_8: for (int i = 0; i < 8; i++)
    {
        if (i < ((8 / 2) / 2) * ((8 / 2) / 2) * ((8 / 2) / 2))
        {
    ...
    if (l3_has_data)
    {
        VITIS_LOOP_189_9: for (int i = 0; i < 8; i++)
        {
...
# 232 "streaming_bitmap_constructor.cpp"
    VITIS_LOOP_232_10: for (int i = 0; i < 8; i++)
    {
    ...
    VITIS_LOOP_244_11: for (int i = 0; i < 16; i++)
    {
    ...
    VITIS_LOOP_256_12: for (int i = 0; i < 16; i++)
    {
