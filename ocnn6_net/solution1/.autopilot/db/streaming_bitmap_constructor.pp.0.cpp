# 1 "streaming_bitmap_constructor.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "streaming_bitmap_constructor.cpp" 2
...
# 2 "streaming_bitmap_constructor.cpp" 2
# 11 "streaming_bitmap_constructor.cpp"
void process_voxel(
 ...
 if (current_voxel.occupancy)
    {
        ...
        ap_uint<32> l0_bit_idx = coord_to_idx(x, y, z, 8);
        ...
        ap_uint<32> l1_idx = block_z * (8 / 2) * (8 / 2) + block_y * (8 / 2) + block_x;
        L1_temp[l1_idx] = 1;
...
#pragma HLS INTERFACE s_axilite port = return
 static ap_uint<32> voxel_count = 0;
    ...
    static ap_uint<1> l0_block_buffer[2][8][8];
    static ap_uint<1> l1_block_buffer[2][(8 / 2)][(8 / 2)];
    static ap_uint<1> l2_temp_buffer[((8 / 2) / 2)][((8 / 2) / 2)];
    static ap_uint<32> current_z_layer = 0;
...
#pragma HLS ARRAY_PARTITION variable=l0_block_buffer dim=1 complete
#pragma HLS ARRAY_PARTITION variable=l1_block_buffer dim=1 complete
 static ap_uint<1> initialized = 0;
    if (!initialized)
    {
        ...
        VITIS_LOOP_111_1: for (int z = 0; z < 2; z++) {
            VITIS_LOOP_112_2: for (int y = 0; y < 8; y++) {
                VITIS_LOOP_113_3: for (int x = 0; x < 8; x++) {
                    l0_block_buffer[z][y][x] = 0;
                }
            }
        }
        VITIS_LOOP_118_4: for (int z = 0; z < 2; z++) {
            VITIS_LOOP_119_5: for (int y = 0; y < (8 / 2); y++) {
                VITIS_LOOP_120_6: for (int x = 0; x < (8 / 2); x++) {
                    l1_block_buffer[z][y][x] = 0;
                }
            }
        }
        VITIS_LOOP_125_7: for (int y = 0; y < ((8 / 2) / 2); y++) {
            VITIS_LOOP_126_8: for (int x = 0; x < ((8 / 2) / 2); x++) {
...
VOXEL_PROCESSING:
    while (voxel_count < (8 * 8 * 8) && !voxel_stream.empty())
    {
#pragma HLS PIPELINE II = 1
 VoxelData voxel = voxel_stream.read();
        ...
        ap_uint<32> x = voxel.morton_addr & 0x3F;
        ap_uint<32> y = (voxel.morton_addr >> 6) & 0x3F;
        ap_uint<32> z = (voxel.morton_addr >> 12) & 0x3F;


        ap_uint<32> buffer_z = z % 2;
...
L0_TO_L1_PRUNING:
    for (int l1_y = 0; l1_y < (8 / 2); l1_y++)
    {
#pragma HLS PIPELINE II=2
 VITIS_LOOP_169_9: for (int l1_x = 0; l1_x < (8 / 2); l1_x++)
        {
            ...
            VITIS_LOOP_176_10: for (int dz = 0; dz < 2; dz++)
            {
                VITIS_LOOP_178_11: for (int dy = 0; dy < 2; dy++)
                {
                    VITIS_LOOP_180_12: for (int dx = 0; dx < 2; dx++)
                    {
#pragma HLS UNROLL
 ap_uint<32> l0_x = l1_x * 2 + dx;
                        ...
                        ap_uint<1> voxel_bit = (l0_x < 8 && l0_y < 8) ?
            ...
            if (l1_has_data)
            {
                VITIS_LOOP_203_13: for (int i = 0; i < 8; i++)
                {
...
L1_TO_L2_PRUNING:
    for (int l2_y = 0; l2_y < ((8 / 2) / 2); l2_y++)
    {
#pragma HLS PIPELINE II=2
 VITIS_LOOP_216_14: for (int l2_x = 0; l2_x < ((8 / 2) / 2); l2_x++)
        {

            bool l2_has_data = false;
            ap_uint<4> l1_group = 0;

            VITIS_LOOP_222_15: for (int dy = 0; dy < 2; dy++)
            {
                VITIS_LOOP_224_16: for (int dx = 0; dx < 2; dx++)
                {
#pragma HLS UNROLL
 ap_uint<32> l1_x = l2_x * 2 + dx;
                    ...
                    ap_uint<1> l1_bit = (l1_x < (8 / 2) && l1_y_coord < (8 / 2)) ?
            ...
            if (l2_has_data)
            {
                VITIS_LOOP_245_17: for (int i = 0; i < 4; i++)
                {
    ...
    ap_uint<64> l2_complete_level = 0;
    ap_uint<32> l2_bit_pos = 0;

    VITIS_LOOP_258_18: for (int l2_y = 0; l2_y < ((8 / 2) / 2); l2_y++)
    {
        VITIS_LOOP_260_19: for (int l2_x = 0; l2_x < ((8 / 2) / 2); l2_x++)
        {
...
# 319 "streaming_bitmap_constructor.cpp"
    VITIS_LOOP_319_21: for (int i = 0; i < 8; i++)
    {
    ...
    VITIS_LOOP_331_22: for (int i = 0; i < 16; i++)
    {
    ...
    VITIS_LOOP_343_23: for (int i = 0; i < 16; i++)
    {
...
#pragma HLS INLINE
 ...
 MortonAddress addr = extract_morton_address(morton);
    return base_offset * 32 + (addr.row_bits * ((8 * 8 * 8) / (1 << 4)) * 32) + (addr.address_bits * 32);
}
