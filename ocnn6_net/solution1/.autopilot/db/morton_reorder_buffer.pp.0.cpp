...
# 3 "morton_reorder_buffer.cpp" 2
# 12 "morton_reorder_buffer.cpp"
void integrated_morton_reorder_buffer_with_triggers(
    hls::stream<MemRequest> &req_in,
    hls::stream<MortonBitmapInterface> &bitmap_interface,
    hls::stream<RetainedBlockInfo> &retained_blocks_in,
...
#pragma HLS INTERFACE m_axi port = feature_dram depth = 134217728 bundle = gmem_morton max_read_burst_length = 256 max_write_burst_length = 256 num_read_outstanding = 32 num_write_outstanding = 32 latency = 32 offset = slave
...
#pragma HLS INTERFACE s_axilite port = return
 ...
 static MortonBufferEntry buffer[64];
#pragma HLS ARRAY_PARTITION variable = buffer cyclic factor = 16
 static MortonDramMapping mapping_table[64];
#pragma HLS BIND_STORAGE variable = mapping_table type = ram_2p impl = bram
 ...
 static MortonBufferStatus buffer_status;
    ...
    static MemResponse pending_responses[64];
    static ap_uint<1> response_valid[64];
...
PROCESS_MORTON_WRITE_REQUESTS:
    while (!req_in.empty())
    {
        ...
        if (req.is_write && req.should_store)
        {
            ...
            ap_uint<32> voxel_idx = req.request_id / 32;
            ap_uint<32> feature_idx = req.request_id % 32;
            ap_uint<32> write_addr = morton_to_dram_address(req.morton_addr, feature_idx);

            feature_dram[write_addr] = req.data;


            static ap_uint<60> last_morton = 0;
...
# 111 "morton_reorder_buffer.cpp"
    bool more_data_available = true;


    VITIS_LOOP_114_1: while (more_data_available)
    {

        VITIS_LOOP_117_2: while (!retained_blocks_in.empty() && buffer_fill < 64)
        {
#pragma HLS PIPELINE II = 1
 RetainedBlockInfo retained = retained_blocks_in.read();
            ...
            VITIS_LOOP_180_3: for (int i = 0; i < 8; i++)
            {
                if (block_voxels[i])
                {
        ...
        if (buffer_fill > 0)
        {
            ...
            VITIS_LOOP_225_4: for (int i = 0; i < buffer_fill; i++)
            {
                buffer[i].valid = 0;
            }
        ...
        VITIS_LOOP_235_5: while (!response_in.empty())
        {
...
PROCESS_RETAINED_LOOP:
    while (!retained_blocks.empty() && buffer_fill < 64)
    {
#pragma HLS PIPELINE II = 1
...
SORT_BY_ROW_BITS:
    ...
    {
        VITIS_LOOP_349_1: for (int j = 0; j < buffer_fill - i - 1; j++)
        {
#pragma HLS PIPELINE II = 1
...
#pragma HLS PIPELINE II = 1
 if (!buffer[i].valid || !buffer[i].is_retained)
        ...
        ap_uint<32> dram_addr = 0 + buffer[i].dram_offset;


        const ap_uint<32> MAX_SAFE_BURST = 8;
        if (burst_len == 0 || (dram_addr == burst_start + burst_len * 8 * 32 && burst_len < MAX_SAFE_BURST))
        {
        ...
        else
        {
            ...
            BURST_READ:
                ap_uint<32> burst_size = burst_len * 8 * 32;
                if (burst_size > 64) burst_size = 64;
                VITIS_LOOP_404_2: for (int j = 0; j < burst_size; j++)
                {
#pragma HLS UNROLL factor = 4
 ap_uint<32> read_addr = burst_start + j;
                    if (read_addr < 1048576) {
                        burst_data[j] = feature_dram[read_addr];
                    }
                }


                VITIS_LOOP_414_3: for (int k = processed_entries; k < processed_entries + burst_len; k++)
                {
                    MemResponse response;
                    response.request_id = k;
                    response.data = burst_start + (k - processed_entries) * 8 * 32;
                    if (!resp_out.full())
                    {
                        resp_out.write(response);
                    }
    ...
    {
    FINAL_BURST:
        ap_uint<32> final_burst_size = burst_len * 8 * 32;
        if (final_burst_size > 64) final_burst_size = 64;
        VITIS_LOOP_439_4: for (int j = 0; j < final_burst_size; j++)
        {
#pragma HLS UNROLL factor = 4
 ap_uint<32> read_addr = burst_start + j;
            if (read_addr < 1048576) {
        ...
        VITIS_LOOP_448_5: for (int k = processed_entries; k < processed_entries + burst_len; k++)
        {
            MemResponse response;
            response.request_id = k;
