...
void ocnn6_streaming_layer_pipeline(
    ...
    float *layer_bias,
    ap_uint<512> *L3_bitmap,
    ap_uint<512> *L2_bitmap,
    ap_uint<512> *L1_bitmap,
    ap_uint<512> *L0_bitmap,
    PrunedBitmapInfo &bitmap_info,
    ap_uint<32> *feature_dram_read,
    ap_uint<32> *feature_dram_write
);
...
__attribute__((sdx_kernel("ocnn6_net_8_layer_pipeline", 0))) void ocnn6_net_8_layer_pipeline(
    hls::stream<VoxelData> &input_voxel_stream,
...
struct LayerZLayerVoxel {
    ...
    ap_uint<32> x, y;
    ap_uint<1> valid;
    ap_uint<32> dram_address;

};

struct LayerZLayerBuffer {
    LayerZLayerVoxel voxels[(8 * 8 * 8)];
    ap_uint<32> voxel_count;
    ap_uint<32> z_layer;
    ap_uint<1> loaded;
};
...
#line 54 "/home/robin/Desktop/ocnn-project/run_ocnn6.tcl"
#pragma HLSDIRECTIVE TOP name=ocnn6_net_8_layer_pipeline
# 52 "ocnn6_net.cpp"
...
# 101 "ocnn6_net.cpp"
 hls::stream<VoxelData> feature_data_stream("bitmap_features");
    hls::stream<ap_uint<60>> write_addr_stream("bitmap_addrs");
...
#pragma HLS STREAM variable=early_trigger_stream depth=64
 ...
 ap_uint<32> initial_processed_voxels = 0;
    ...
    VITIS_LOOP_131_1: while (!feature_data_stream.empty() && !write_addr_stream.empty()) {
#pragma HLS PIPELINE II=1
 VoxelData voxel = feature_data_stream.read();
        ap_uint<60> morton_addr = write_addr_stream.read();


        VITIS_LOOP_137_2: for (int c = 0; c < 3; c++) {
#pragma HLS UNROLL factor=2
 ap_uint<32> dram_addr = pruned_voxel_count * 3 + c;
...
#pragma HLS PIPELINE II=1
 VoxelData initial_voxel;
        ...
        VITIS_LOOP_179_3: for (int c = 0; c < 3; c++) {
#pragma HLS UNROLL factor=2
 ap_uint<32> dram_addr = v * 3 + c;
        ...
        VITIS_LOOP_186_4: for (int c = 3; c < 32; c++) {
#pragma HLS UNROLL factor=2
 initial_voxel.features[c] = 0.0f;
        }


        initial_voxel.morton_addr = v;
    ...
    ap_uint<60> current_morton_list[(8 * 8 * 8)];
    ap_uint<32> current_pruned_voxels_count = current_pruned_voxels;


    VITIS_LOOP_209_5: for (ap_uint<32> v = 0; v < current_pruned_voxels && v < (8 * 8 * 8); v++) {
        current_morton_list[v] = v;
    }
...
INIT_PIPELINE_STREAM:
    for (ap_uint<32> v = 0; v < current_pruned_voxels_count && v < (8 * 8 * 8); v++)
    {
#pragma HLS PIPELINE II=1
 VoxelData initial_voxel;


        VITIS_LOOP_232_6: for (int c = 0; c < 3; c++) {
            ap_uint<32> dram_addr = v * 3 + c;
            ap_uint<32> feature_word = pruned_feature_dram_read[dram_addr];
            initial_voxel.features[c] = *((float *)&feature_word);
        }
        VITIS_LOOP_237_7: for (int c = 3; c < 32; c++) {
            initial_voxel.features[c] = 0.0f;
        }
 ...
 static ZBufferConvolutionEngine layer_conv_engine;
    ...
    if (!engine_initialized) {
        layer_conv_engine.initialize_z_buffer(
            reinterpret_cast<float(*)[32][32]>(layer_weights),
            layer_bias);
...
#pragma HLS ARRAY_PARTITION variable=input_features complete
 VITIS_LOOP_460_1: for (int c = 0; c < 32; c++) {
...
#pragma HLS ARRAY_PARTITION variable=output_features complete
 VITIS_LOOP_468_2: for (int c = 0; c < 32; c++) {
#pragma HLS UNROLL factor=8
...
#pragma HLS ARRAY_PARTITION variable=neighbor_features complete
 ...
 if (neighbor_exists) {
                ...
                VITIS_LOOP_496_3: for (int f = 0; f < 32; f++) {
#pragma HLS UNROLL factor=8
 ap_uint<32> dram_addr = morton_to_dram_address(neighbor_morton, f);
                ...
                VITIS_LOOP_504_4: for (int f = 0; f < 32; f++) {
#pragma HLS UNROLL factor=8
...
#pragma HLS UNROLL factor=8
 output_features[f] += neighbor_contribution[f];
        ...
        VITIS_LOOP_531_6: for (int c = 0; c < 32; c++) {
#pragma HLS UNROLL factor=8
...
#pragma HLS INLINE
 ...
 ap_uint<60> morton_addr = morton3D(x, y, z);
    ...
    return morton_addr % bitmap_info.total_size;
}
...
#pragma HLS PIPELINE II=1
 ...
 if (output_stream.empty()) break;

        VoxelData output_voxel = output_stream.read();
        voxel_count++;


        ap_uint<32> x = output_voxel.morton_addr & 0x3F;
        ap_uint<32> y = (output_voxel.morton_addr >> 6) & 0x3F;
        ap_uint<32> z = (output_voxel.morton_addr >> 12) & 0x3F;
