...
const ap_int<8> ConvolutionEngine::NEIGHBOR_OFFSETS[(3 * 3 * 3)][3] = {
...
MORTON_READ_SORT:
    ...
    {
        VITIS_LOOP_27_1: for (ap_uint<32> j = 0; j < num_voxels - i - 1; j++)
        {
...
#pragma HLS INLINE
 ...
 VITIS_LOOP_65_1: for (int f = 0; f < 32 && f < 8; f++)
    {
#pragma HLS UNROLL
...
#pragma HLS UNROLL factor=2
 active[v] = 0;
        VITIS_LOOP_83_1: for (int f = 0; f < 32; f++)
        {
...
#pragma HLS ARRAY_PARTITION variable=accumulator cyclic factor=8 dim=2
 ...
 if (voxel_slot >= (8 * 8)) return;


INIT_BIAS:
    for (int out_f = 0; out_f < 32; out_f++)
    {
#pragma HLS UNROLL factor=4
...
#pragma HLS PIPELINE II=2
 VITIS_LOOP_118_1: for (int in_f = 0; in_f < 32; in_f++)
        {
#pragma HLS UNROLL factor=4
...
#pragma HLS UNROLL factor=16
 VITIS_LOOP_159_1: for (int y = 0; y < 8; y++) {
#pragma HLS UNROLL factor=16
...
#pragma HLS LOOP_TRIPCOUNT min = 1 max = 262144
 ...
 ap_uint<60> morton = morton_list[v];
        ap_uint<32> x, y, z;
        morton_decode(morton, x, y, z);

        if (z == target_z && x < 8 && y < 8)
        {
...
#pragma HLS INLINE
 ...
 ap_uint<32> neighbor_x = center_x + dx;
    ...
    if (neighbor_x >= 8 || neighbor_y >= 8 || neighbor_z >= 8)
        return false;


    int map_index;
    if (dz == -1) map_index = 0;
    else if (dz == 0) map_index = 1;
    else if (dz == 1) map_index = 2;
    else return false;
...
RESET_SPATIAL_COVERAGE:
    ...
    {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_248_1: for (int y = 0; y < 8; y++)
        {
#pragma HLS PIPELINE II=1
 active[x][y] = 0;
            VITIS_LOOP_252_2: for (int f = 0; f < 32; f++)
            {
#pragma HLS UNROLL factor=2
...
#pragma HLS ARRAY_PARTITION variable=mult_units cyclic factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=input_features cyclic factor=4
#pragma HLS ARRAY_PARTITION variable=output_features cyclic factor=4


 if (voxel_x >= 8 || voxel_y >= 8) return;
...
PER_CHANNEL_CONVOLUTION:
    for (int out_f = 0; out_f < 32; out_f++)
    {
        ...
        VITIS_LOOP_292_1: for (int in_f = 0; in_f < 32; in_f++)
        {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_295_2: for (int z = 0; z < 3; z++)
            {
#pragma HLS UNROLL
 float mult_result = input_features[in_f] * weights[out_f][in_f];
#pragma HLS RESOURCE variable=mult_result core=DSP48 impl=dsp
 mult_units[in_f][voxel_x][voxel_y][z] = mult_result;
                local_sum += mult_result;
            }
        }
        accumulator[voxel_x][voxel_y][out_f] = local_sum;
    }
...
STORE_WEIGHTS:
    ...
    {
        VITIS_LOOP_326_1: for (int i = 0; i < 32; i++)
        {
#pragma HLS PIPELINE II = 1
 VITIS_LOOP_329_2: for (int j = 0; j < 32; j++)
            {
#pragma HLS UNROLL factor = 8
...
#pragma HLS UNROLL
 occupancy_maps[z].loaded = 0;
        ...
        VITIS_LOOP_377_3: for (int x = 0; x < 8; x++) {
#pragma HLS UNROLL factor=8
 VITIS_LOOP_379_4: for (int y = 0; y < 8; y++) {
#pragma HLS UNROLL factor=8
...
#pragma HLS INLINE off
 ...
 ap_uint<32> voxel_slot = output_index % (8 * 8);

    float partial_sums[32];
...
#pragma HLS LOOP_TRIPCOUNT min = 27 max = 27
 ...
 int dx = NEIGHBOR_OFFSETS[n][0];
        ...
        if (neighbor_exists && neighbor_feature_index < num_voxels)
        {
            ...
            int mult_array_idx = 1;
            if (dz == -1) mult_array_idx = 0;
            else if (dz == 1) mult_array_idx = 2;


            float neighbor_features[32];
#pragma HLS ARRAY_PARTITION variable=neighbor_features complete
 ...
 LOAD_NEIGHBOR_FEATURES:
            ...
            {
#pragma HLS UNROLL factor = 8
 ap_uint<60> neighbor_morton = morton_list[neighbor_feature_index];
...
#pragma HLS UNROLL factor = 8
 float result = (partial_sums[f] > 0.0f) ? partial_sums[f] : 0.0f;
        ap_uint<32> output_addr = ((8 * 8 * 8) * 32) +
                                  (output_index * 32) + f;
...
void SystolicArray::load_weights(float weights[32][32])
{
#pragma HLS INLINE off
LOAD_WEIGHTS_OUTER:
    for (int i = 0; i < 32; i++)
...
void SystolicArray::compute(float input_features[32],
                            float output_features[32],
                            float bias[32])
{
#pragma HLS INLINE off
...
#pragma HLS PIPELINE II = 1
 PE_COMPUTE:
        ...
        {
#pragma HLS UNROLL
 VITIS_LOOP_684_1: for (int j = 0; j < 32; j++)
            {
#pragma HLS UNROLL
 if (k >= j && k - j < 32 && (k - j) == i)
                {
...
#pragma HLS UNROLL
 float sum = 0;
        VITIS_LOOP_700_2: for (int i = 0; i < 32; i++)
        {
#pragma HLS UNROLL
...
RESET_PE:
    ...
    {
#pragma HLS UNROLL
 VITIS_LOOP_716_1: for (int j = 0; j < 32; j++)
        {
#pragma HLS UNROLL
 pe_array[i][j].accumulator = 0;
        }
...
void ConvolutionEngine::initialize(float weights[(3 * 3 * 3)][32][32],
                                   float bias[32])
{
#pragma HLS INLINE off
STORE_WEIGHTS:
    for (int n = 0; n < (3 * 3 * 3); n++)
    {
        VITIS_LOOP_730_1: for (int i = 0; i < 32; i++)
        {
#pragma HLS PIPELINE II = 1
 VITIS_LOOP_733_2: for (int j = 0; j < 32; j += 8)
            {
#pragma HLS UNROLL factor = 8
 VITIS_LOOP_736_3: for (int k = 0; k < 8 && (j + k) < 32; k++)
                {
#pragma HLS UNROLL
...
{
#pragma HLS INLINE off
 ap_uint<60> current_morton = morton_list[feature_idx];
    ap_uint<32> x, y, z;
    morton_decode(current_morton, x, y, z);
    float partial_sums[32];
#pragma HLS ARRAY_PARTITION variable = partial_sums complete
 VITIS_LOOP_795_1: for (int f = 0; f < 32; f++)
    {
#pragma HLS UNROLL
 partial_sums[f] = stored_bias[f];
    }
